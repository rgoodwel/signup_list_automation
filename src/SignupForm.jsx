...
export default function SignupForm({ players, weekKey: propWeekKey, week: propWeek, onSignedUp }) {
  ...
  const { openHoleCount, allowBGroups, bGroupThreshold } = getLeagueSignupConfig(league)
  ...
  const nonEmptyBGroupHoles = Object.keys(holes || {}).reduce(
    (sum, holeKey) => sum + (holeKey.endsWith('B') && (holes[holeKey]?.length ?? 0) > 0 ? 1 : 0),
    0,
  )
  const computedBHoleCount = allowBGroups
    ? Math.max(0, MIN_FULLY_AVAILABLE_HOLES - (openHoleCount - nonEmptyAGroupHoles) + nonEmptyBGroupHoles)
    : 0
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!name.trim() || emailRequired || phoneRequired) {
        ...
        if (emailRequired) missingFields.push('email address')
        if (phoneRequired) missingFields.push('phone number')
        
        showError(
          'Missing Information',
          `Please fill in your ${missingFields.join(', ')} before signing up.`,
    ...
    if (result.ok) {
      ...
      } else if (reason.includes('not available for this league')) {
        title = 'Group B Unavailable'
        hint = 'This league does not allow Group B holes.'
        ...
        <>
          ...
          {allowBGroups && !bUnlocked && week && bUnlockRemaining > 0 && (
            <p className="b-group-notice">
              Group B holes begin to unlock when {bUnlockRemaining} more empty hole(s){bUnlockRemaining !== 1 ? 's' : ''} have a player assigned.
            </p>
          )}
          {bUnlocked && (
            <>
